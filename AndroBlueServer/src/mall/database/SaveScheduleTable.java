...
public class SaveScheduleTable {
    public static void saveScheduleTable(ArrayList<ScheduleModel> list)
    {
        try{
            ...
            PreparedStatement psmt1= connection.prepareStatement("delete from schedule_table where D_ID=3");
            psmt1.execute();
            psmt1.close();
            for(Object obj:list)
            {
                ...
                {
                    psmt1= connection.prepareStatement("insert into schedule_table(D_ID,NAME,inTime,OUTTIME,Days) values(3,?,?,?,?)");
